import alluser from "../../Db/Dbconnecting/DbModel/DbModel.js";
import bcrypt from 'bcryptjs';
import jwt from 'jsonwebtoken';
import crypto from 'crypto';
import sendMailVerification from "../../Gmail/Email/Email.js";
import Application from "../../Db/Dbconnecting/DbModel/ApplicationModel.js";
...
const ForgotPasswordController = async (req, res) => {
    try {
      ...
      try {
        await sendMailVerification(email, sendingForgotPasswordLink);
      } catch (error) {
        console.error('Error sending email:', error);
        return res.status(500).send({ status: 500, message: 'Error sending reset password email' });
      }
